 ...
 $otp=$_POST["otp"];
     //$q="SELECT * FROM otp_expiry WHERE otp='". $_POST["otp"] ."' AND is_expired !=1 AND NOW() <= DATE_ADD(create_at, INTERVAL 24 HOUR)";
         ...
         //$result = mysqli_query($conn,"SELECT * FROM otp_expiry WHERE otp='$otp'");
          ...
          $result = mysqli_query($conn,"SELECT * FROM otp_expiry WHERE otp='$otp' AND is_expired !=1 AND NOW() <= DATE_ADD(create_at, INTERVAL 24 HOUR)");
	if($result>0) {
...
<form name="" method="post" >
  <header><h3>An OTP Is Sent To Your Email, Get IT and Enter Here To Login</h3></header>
  <p style="color:red;"></p>
